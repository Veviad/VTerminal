...
//!
//! VTerminal patch: expose explicit embedded-MTP loading. See VTERMINAL_PATCH.md.
...
impl LlamaModelParams {
    /// Whether embedded MTP tensors are loaded with the target model.
    ///
    /// llama.cpp 0.1.156 defaults this to false. Creating an MTP context after
    /// skipping these tensors can abort the process instead of returning an error.
    #[must_use]
    pub fn load_mtp(&self) -> bool {
        self.params.load_mtp
    }

    /// Load embedded MTP tensors before creating an MTP draft context.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn embedded_mtp_loading_is_explicit() {
        let params = LlamaModelParams::default();
        assert!(!params.load_mtp());
        let params = params.with_load_mtp(true);
        assert!(params.load_mtp());
        assert!(!params.with_load_mtp(false).load_mtp());
    }

    #[test]
    fn an_explicit_empty_device_list_disables_automatic_gpu_selection() {
        let automatic = LlamaModelParams::default();
        assert!(automatic.params.devices.is_null());
        let cpu = automatic.with_devices(&[]).unwrap();
        assert!(!cpu.params.devices.is_null());
        // SAFETY: with_devices points into cpu's owned pinned array, which
        // remains alive here; the non-null array always has a first element.
        assert!(unsafe { *cpu.params.devices }.is_null());
    }
