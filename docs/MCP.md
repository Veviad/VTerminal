...
VTerminal can use tools from several Model Context Protocol servers in Ask, Agent and the Chat workspace. Ask receives only MCP tools; Chat adds Knowledge search; Agent also retains its terminal and Knowledge tools. Every MCP call asks for approval unless you explicitly remember that exact tool for that exact server and schema.
...
VTerminal implements MCP tools. Resources, prompts, roots, sampling, elicitation, Tasks, and MCP Apps are deferred. An input_required or background-task response is shown as unsupported instead of silently continuing.
