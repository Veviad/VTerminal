...
- **Default terminal AI mode**: **Settings → Agent → Default mode** lets you choose **Always Ask**, **Always Agent**, or **Remember last used** for startup, reopened terminal sessions, new terminals, and new terminal chats. Remember last used saves your latest Ask/Agent selection across terminals and app restarts. Always Ask remains the initial default. Command approval starts at Ask for each new or restored conversation.
...
- **Model Context Protocol**: select several remote Streamable HTTP or sandboxed local stdio servers per chat, optionally connect chosen servers when VTerminal starts, use their tools in Ask, Agent and Chat, and keep every tool call behind an explicit approval. OAuth 2.1, bearer tokens, custom headers, per-chat tool switches, defaults, and Claude/VS Code JSON import are built in. See [MCP setup and security](docs/MCP.md).
