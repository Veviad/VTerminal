...
- **Model Context Protocol**: select several remote Streamable HTTP or sandboxed local stdio servers per chat, optionally connect chosen servers when VTerminal starts, use their tools in Ask and Agent, and keep every tool call behind an explicit approval. OAuth 2.1, bearer tokens, custom headers, per-chat tool switches, defaults, and Claude/VS Code JSON import are built in. See [MCP setup and security](docs/MCP.md).
