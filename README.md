...
**[Download 0.1.5 for macOS](https://github.com/Veviad/VTerminal/releases/download/v0.1.5/VTerminal_0.1.5_aarch64.dmg)** (Apple Silicon, 11 MB) · **[vterminal.veviad.com](https://vterminal.veviad.com)**
...
> **Status: early.** VTerminal is pre-1.0 (`0.1.5`) and developed in the open. It is used daily by its author, but expect rough edges and breaking changes between versions.
...
- **Reusable Runbooks** *(experimental and disabled by default)* — versioned YAML checklists with immutable run snapshots, per-mutation approvals, visible-terminal execution, evidence, and canonical JSON/Markdown reports. See [the authoring guide](docs/RUNBOOKS.md).
